...
export default function HotelDetail() {
  ...
  if (!hotel) return (
    <div style={{ paddingTop: '8rem', textAlign: 'center' }}>
      ...
      <Link to="/hotels" className="btn btn-outline" style={{ marginTop: '1.5rem', display: 'inline-flex' }}>← Back to Hotels</Link>
    </div>
  )

  const { name, description, star_rating, city, country, price_per_night, currency = 'KES',
    featured_image, gallery_images = [], amenities = [], total_rooms, seo_title, seo_description } = hotel
  ...
  return (
    <>
      ...
      <div style={{ paddingTop: '5rem', position: 'relative', height: '60vh', overflow: 'hidden' }}>
        <img src={featured_image || PLACEHOLDER} alt={name}
        ...
        <div className="container" style={{ position: 'absolute', bottom: '2.5rem', left: '50%', transform: 'translateX(-50%)', width: '100%' }}>
          <div style={{ display: 'flex', gap: '3px', marginBottom: '0.75rem' }}>
            {Array.from({ length: star_rating || 5 }).map((_, i) => <Star key={i} size={16} fill="var(--gold)" color="var(--gold)" />)}
          </div>
      ...
      <div className="container" style={{ padding: '3rem clamp(1rem,4vw,2.5rem)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '3rem', alignItems: 'start' }}>
          <div>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.6rem', marginBottom: '1rem' }}>About the Property</h2>
            ...
            {amenities.length > 0 && (
              <div>
                <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.3rem', marginBottom: '1rem' }}>Amenities</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '0.5rem' }}>
                  {amenities.map((a) => (
                    <div key={a} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', padding: '0.6rem 0.9rem', background: 'var(--dark-2)', border: '1px solid rgba(255,255,255,0.06)', fontSize: '0.82rem', color: 'var(--gray-mid)' }}>
                      <Wifi size={13} color="var(--gold)" /> {a}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={{ position: 'sticky', top: '6rem' }}>
            <div style={{ background: 'var(--dark-2)', border: '1px solid rgba(201,168,76,0.2)', padding: '2rem', marginBottom: '1.5rem' }}>
              ...
              <div style={{ fontSize: '0.78rem', color: 'var(--gray-muted)', marginBottom: '1.5rem', letterSpacing: '0.05em' }}>per night · {total_rooms} rooms available</div>
              <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '1.5rem' }}>
                {Array.from({ length: star_rating || 5 }).map((_, i) => <Star key={i} size={14} fill="var(--gold)" color="var(--gold)" />)}
              </div>
              <button className="btn btn-gold w-full" style={{ marginBottom: '0.75rem', justifyContent: 'center' }}>
                Check Availability
              </button>
              <Link to="/contact" className="btn btn-ghost w-full" style={{ justifyContent: 'center', display: 'flex' }}>
                Contact Hotel
              </Link>
            </div>

            <div style={{ background: 'var(--dark-2)', border: '1px solid rgba(201,168,76,0.2)', padding: '2rem' }}>
